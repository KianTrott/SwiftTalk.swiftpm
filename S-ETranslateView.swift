//
//  S-ETranslateView.swift
//  SwiftTalk
//
//  Created by Kian J. Trotter on 4/29/26.
//
        ...
        VStack{
            HStack{
                ...
                Button {
                    ...
                    let url = URL(string: "https://api.mymemory.translated.net/get?q=\(spanishText)&langpair=es|en")!
                                  
                    let session = URLSession.shared.dataTask(with: url) { data, response, error in
                        if let data = data {
                            if let json = try? JSONSerialization.jsonObject(with: data) as? NSDictionary {
                                let dictionary = json["responseData"] as? NSDictionary
                                englishText = (dictionary?["translatedText"] as! String)
                            }
                        }
                    }.resume()
